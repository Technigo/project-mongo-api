...
if(process.env.RESET_DB) {
  const resetDataBase = async () => {
    ...
    // const testUser = new User({name: "Caroline", age: 35, deceased: false})
    // testUser.save()
  }
  resetDataBase()
}
...
app.get("/songs/", async (req, res) => {
  ...
  try {
    // if (req.params.genre && req.params.danceability) {
      response.body = await Song.find({genre: genreQuery, danceability:
         danceabilityQuery}).limit(2).sort({energy: -1}).select({trackName: 1, artistName:1})
