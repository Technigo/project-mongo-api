...
//   try {
//     const allArtists =  (await Song.find()).projection({ artistName: 1 });
//     if (allArtists) {
//       res.status(200).json({
//         success: true,
//         body: {
...
//         success: false,
//         body: {
//           message: "No artists in the list"
//         }
//       })
//     }
//   } catch(e) {
...
app.get("/songs/artists", async (req, res) => {
  try {
...
app.get("/songs/artists/name", async (req, res) => {
  try {
    const artistList = await Song.find().populate('artistName');
    if (artistList) {
      res.status(200).json({
        success: true,
        body: artistList
